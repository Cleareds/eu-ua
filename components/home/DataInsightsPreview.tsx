...
export default function DataInsightsPreview() {
  const lastTrade = euData.tradeShare[euData.tradeShare.length - 1];
  const lastStudents = euData.students[euData.students.length - 1];
  const lastAssist = euData.assistance[euData.assistance.length - 1];
  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
        <MiniStat
          label={`EU Share of Ukraine Exports (${lastTrade.year})`}
          value={`${lastTrade.value}%`}
          change="↑ from 26% in 2013"
          color="#003399"
        />
        <MiniStat
          label={`Ukrainian Students in EU (${lastStudents.year})`}
          value={`${(lastStudents.value / 1000).toFixed(0)}K`}
          change="↑ 8× increase since 2015"
          color="#003399"
        />
        <MiniStat
          label={`EU Financial Support (${lastAssist.year})`}
          value={`€${lastAssist.value}B`}
          change="↑ largest ever EU support package"
          color="#003399"
        />
      </div>
